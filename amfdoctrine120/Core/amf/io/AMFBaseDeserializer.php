...
class AMFBaseDeserializer {
    ...
    function mapClass($typeIdentifier)
    {
		...
		/*******
		MODIFIED: the below if statement is a modification of the ORIGINAL.  It apparently made Doctrine + AMFPHP work, but it broke at some point.
		***********/
		if($include !== FALSE)
		{
			require('/var/www/amfdoctrinedevel/bootstrapper.php');
			
			$doctrineStyleName = str_replace('/', '\\', $mappedClass);
			//if(class_exists($classname))
			//{
				$clazz = new $doctrineStyleName;
			//}
		}
		/*****
		ORIGINAL of the above if statement: modified for Doctrine
		if($include !== FALSE)
		{
