...
class UserService {
	...
	public function getUserById($userId) {
		...
		// create the select query and execute
		$dql = "SELECT u FROM org\cim\User u WHERE u.id=$userId";
		$query = $entityManager->createQuery($dql);
		$users = $query->getResult();
	...
	public function attemptLogin($password) {
		...
		if(sizeof($users) != 1) {
			...
			$users[0]->lastLoginDate = new DateTime('now', $timezone);
...
//		return $user;
	...
	public function saveUser($user) {
		...
		if($newUser) {
			sendWelcomeMail($user->email, $user->id);
		}
